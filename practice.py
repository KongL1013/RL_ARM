...
bigtrain_ang_path='/home/dd/PycharmProjects/sim_arm_net/train-data/bigtrain_ang_data.xls'
bigtrain_pos_path='/home/dd/PycharmProjects/sim_arm_net/train-data/bigtrain_pos_data.xls'
...
train_ang_data = excel_to_matrix(bigtrain_ang_path,0)
train_pos_data = excel_to_matrix(bigtrain_pos_path,1)
...
def inference( input_tensor , avg_class , weights1 , biases1 , weights2 , biases2,weights3 , biases3,weights4,biases4,weights5,biases5 ):
    if avg_class==None:
        ...
        layer1 = tf.nn.leaky_relu(tf.matmul(input_tensor,avg_class.average(weights1))+avg_class.average(biases1))
        layer2=tf.nn.leaky_relu(tf.matmul(layer1,avg_class.average(weights2))+avg_class.average(biases2))
        layer3=tf.nn.leaky_relu(tf.matmul(layer2,avg_class.average(weights3))+avg_class.average(biases3))
        layer4 = tf.nn.leaky_relu(tf.matmul(layer3, avg_class.average(weights4)) + avg_class.average(biases4))
        resu=tf.matmul(layer4,avg_class.average(weights5))+avg_class.average(biases5)#没有softmax层
    return resu
...
def train():
    ...
    regularization = regularizer(weights1)+regularizer(weights2)+regularizer(weights3)+regularizer(weights4)+regularizer(weights5)
    ...
    with tf.Session() as sess:
        ...
        for i in range(TRAINING_STEPS):
            ...
            if i%1000== 0:
                ...
                print(i)
                cc=[[y_[70],y[70],y_[150],y[150],y_[45],y[45],y_[100],y[100]]]
                print(sess.run(cc,feed_dict={x: val_ang_data, y_: val_pos_data}))
